// src/tooling/cli-commands/w2-derive-digital.ts

/**
 * w2:derive-digital CLI Command
 *
 * Digital PDF derivation step for W2 PDF workflow.
 * Converts print PDF to digital-optimized version with hyperlinks.
 *
 * Usage:
 *   Generate prompt: pnpm w2:derive-digital --run <id>
 *   Save result:     pnpm w2:derive-digital --save --run <id> --pdf <path>
 */
...
import { generateDeriveDigitalPrompt } from '../w2/prompt-generator.js';
...
if (values.help) {
  console.log(
    CLIFormatter.format({
      title: 'W2 DERIVE DIGITAL - HELP',
      content: `Digital PDF derivation step for W2 PDF workflow.
...
if (!runId) {
  console.error(
    CLIFormatter.format({
      ...
      nextStep: ['Usage:', '  pnpm w2:derive-digital --run <id>'],
    })
  );
  process.exit(1);
}
...
async function runGenerateMode(db: Database.Database): Promise<void> {
  console.log(CLIFormatter.header('W2 DERIVE DIGITAL PDF'));
  ...
  // Find print PDF
  const printPdfPath = resolve(projectRoot, `data/w2-artifacts/${runId}/draft.pdf`);
  if (!existsSync(printPdfPath)) {
    console.error(`ERROR: Print PDF not found: ${printPdfPath}`);
    console.error('Run w2:create-pdf first to generate a print PDF');
    process.exit(1);
  }
  ...
  const promptPath = promptWriter.writeDeriveDigitalPrompt(prompt);

  console.log(
    CLIFormatter.format({
      title: 'DERIVE DIGITAL PROMPT GENERATED',
      content: CLIFormatter.table([
        { key: 'Run ID', value: runId! },
        { key: 'Book', value: `${book.title} (${book.slug})` },
        { key: 'Print PDF', value: printPdfPath },
        { key: 'Digital PDF', value: digitalPdfPath },
      ...
      nextStep: [
        ...
        `  pnpm w2:derive-digital --save --run=${runId} --pdf=${digitalPdfPath}`,
      ],
    })
  );
}
...
async function runSaveMode(db: Database.Database): Promise<void> {
  ...
  console.log(CLIFormatter.header('W2 DERIVE DIGITAL - SAVE'));
  console.log(`Run ID: ${runId}`);
  console.log(`Digital PDF: ${resolvedPath}`);
  console.log('');

  // Register digital PDF
  const resultSaver = new W2ResultSaver(db, runId!);
  resultSaver.saveDigitalPdf(resolvedPath);

  console.log(
    CLIFormatter.format({
      title: 'DIGITAL PDF SAVED',
      content: CLIFormatter.table([
        { key: 'Run ID', value: runId! },
        { key: 'Digital PDF', value: resolvedPath },
      ]),
      status: [{ label: 'Digital PDF registered', success: true }],
      nextStep: [
        'Next step:',
        `  pnpm w2:finalize --run=${runId}`,
      ],
    })
  );
}
